...
|-----------|---------|--------|
| `transactions` | All on-chain tx records | `saveTransaction()` (server) |
| `activity` | Cross-pillar activity feed | `writeActivity()` (server) |
| `treasury_logs` | Revenue/accounting observer | `logTreasuryEvent()` (server) |
...
| `ai_sessions` | Chat session persistence | AI chat route |
| `saved_prompts` | User prompt library | Workspace routes |
| `credit_balances` | Credit ledger | Credits engine |
...
**AI OS:**
- `POST /api/ai/chat` — multi-mode AI with credit pre-check
- `POST /api/ai/copilot` — dynamic-context Copilot

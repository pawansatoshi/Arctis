...
const SOCIALS = [
  ...
  { label: 'Discord',     href: 'https://discord.com/users/965913443687338025', icon: 'DC', color: 'hover:text-indigo-400' },
  { label: 'YouTube',     href: 'https://youtube.com/@PawanSatoshi',     icon: 'YT',       color: 'hover:text-rose-400' },
  { label: 'Telegram',    href: 'https://t.me/pawansatoshiji',            icon: 'TG',       color: 'hover:text-blue-400' },
];
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="flex flex-wrap gap-2 mt-5 pt-5 border-t border-white/[0.06]">
          {SOCIALS.map((s) => (
            <a
              ...
              className={cn(
                'flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-200/50 border border-white/[0.06]',
          ...
          <a
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-200/50 border border-white/[0.06] text-surface-700 text-sm font-medium hover:text-emerald-400 hover:bg-surface-200 transition-all"
          >
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex flex-wrap gap-2">
          {STACK.map((tech) => (
            <span key={tech} className="px-3 py-1.5 rounded-lg bg-surface-200/50 border border-white/[0.06] text-surface-700 text-xs font-medium">
              {tech}
            </span>
          ))}
        </div>

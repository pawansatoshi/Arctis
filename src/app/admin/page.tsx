...
export default function AdminPage() {
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 bg-surface-200/50 rounded-xl border border-black/[0.06] dark:border-white/[0.06] w-fit">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            ...
            className={cn(
              ...
              activeTab === tab.id ? 'bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'text-surface-600 hover:text-surface-950'
            )}
          >
      ...
      {activeTab === 'overview' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: '30D Membership Rev', value: metrics ? `${metrics.membershipRevenue30d.toFixed(2)} USDC` : '—', icon: TrendingUp, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10' },
              { label: '30D Credit Rev', value: metrics ? `${metrics.creditRevenue30d.toFixed(2)} USDC` : '—', icon: Coins, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10' },
              { label: '30D AI Spend', value: metrics ? `${metrics.aiSpend30d.toFixed(4)} USDC` : '—', icon: Zap, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/10' },
              { label: '30D Tx Count', value: metrics ? metrics.txCount30d.toString() : '—', icon: Activity, color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10' },
            ].map((m) => (
      ...
      {activeTab === 'logs' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card overflow-hidden">
          ...
          {logs.length === 0 ? (
            ...
            <div className="divide-y divide-black/[0.04] dark:divide-white/[0.04] max-h-96 overflow-y-auto">
              {logs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-6 py-3 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] font-mono text-xs">
                  <span className={cn('flex-shrink-0 px-1.5 py-0.5 rounded text-xs font-medium mt-0.5',
                    log.level === 'error' ? 'bg-rose-500/20 text-rose-600 dark:text-rose-400' :
                    log.level === 'warn'  ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' :
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-6">
          ...
          <div className="space-y-3">
            {FEATURE_FLAGS.map((flag) => (
              <div key={flag.id} className="flex items-center justify-between py-3 border-b border-black/[0.04] dark:border-white/[0.04]">
                <div className="flex items-center gap-3">
                  {flag.enabled
                    ? <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />

...
const AGENT_TYPES: Record<AgentType, {
  ...
  research: {
    icon: BookOpen, label: 'Research', color: 'text-cyan-600 dark:text-cyan-400', bg: 'bg-cyan-500/10',
  ...
  developer: {
    icon: Code2, label: 'Developer', color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10',
  ...
  engineering: {
    icon: Wrench, label: 'Engineering', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/10',
  ...
  treasury: {
    icon: TrendingUp, label: 'Treasury', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10',
  ...
  monitoring: {
    icon: Eye, label: 'Monitoring', color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10',
  ...
  document: {
    icon: FileText, label: 'Document', color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-500/10',
...
function CreateAgentModal({
  ...
  const [form, setForm] = useState({
    ...
    goals: [''],
    instructions: '',
    model: '', // automatic — ARCTIS routes to the best available AI backend
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} aria-label="Close" className="btn-ghost p-1.5 -mr-1.5">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {step === 'config' && selectedType && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                  ...
                  <div key={i} className="flex gap-2">
                    ...
                    {form.goals.length > 1 && (
                      <button onClick={() => setForm({ ...form, goals: form.goals.filter((_: string, j: number) => j !== i) })}
                        ...
                        className="text-surface-500 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-500/10 transition-colors px-2.5 rounded-lg flex-shrink-0">
                ...
                <button
                  ...
                  className="text-blue-600 dark:text-blue-400 text-xs hover:text-blue-700 dark:hover:text-blue-300 transition-colors"
                >
                  + Add goal
                </button>
              </div>
            ...
            {/* Model — intentionally no picker here. ARCTIS automatically
                routes every agent execution to the best available AI
                backend; there is nothing for the user to choose. */}
...
function AgentDetail({
  agent, walletAddress, onClose, onExecute, onArchive,
}: {
  agent: Agent;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-4 safe-bottom">
      <motion.div
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-black/[0.06] dark:border-white/[0.06]">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button
              onClick={async () => { await onArchive(agent.id, agent.name); onClose(); }}
        ...
        {/* Budget bar */}
        <div className="px-6 py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
              ...
              {(running || output) && (
                <div className="glass-card p-4 min-h-24 max-h-80 overflow-y-auto">
                  <div className="text-surface-600 text-xs mb-2 flex items-center gap-1.5">
                    ...
                    {agent.name} output
                    {running && <Loader2 className="w-3 h-3 animate-spin ml-1 text-blue-600 dark:text-blue-400" />}
                  </div>
                  {output ? <MarkdownContent content={output} /> : null}
                  {running && !output && <span className="text-surface-500 text-sm">Thinking…</span>}
                  {running && output && <span className="inline-block w-1.5 h-4 bg-blue-500 ml-0.5 animate-pulse rounded-sm align-text-bottom" />}
                </div>
              )}
            </div>
          )}
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-surface-500">
                    ...
                    {ex.evaluationVerdict && (
                      <span className={cn('flex items-center gap-1 ml-auto px-1.5 py-0.5 rounded-full text-[10px]',
                        ex.evaluationVerdict === 'PASS' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400')}>
            ...
            <div className="pt-2 space-y-2">
              {reports.length === 0 ? (
                ...
                <details key={report.id} className="glass-card group">
                  ...
                  <div className="px-4 pb-4 border-t border-black/[0.05] dark:border-white/[0.05] pt-3 max-h-64 overflow-y-auto">
...
export default function AgentsPage() {
  ...
  const handleArchive = useCallback(async (agentId: string, name: string) => {
    try {
      ...
      toast.error(`Could not archive ${name} — please try again`);
    }
  }, [fetchAgents]);
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      {isConnected && agents.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.05 }} className="grid grid-cols-1 xs:grid-cols-3 gap-4">

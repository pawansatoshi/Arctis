...
export default function AnalyticsPage() {
  ...
  return (
    <div className="max-w-5xl space-y-6">
        ...
        >
          {[
            { label: 'Total Volume', value: `${totalVolume.toFixed(2)} USDC`, icon: TrendingUp, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10' },
            { label: 'Avg Transfer', value: `${avgAmount.toFixed(2)} USDC`, icon: Activity, color: 'text-cyan-600 dark:text-cyan-400', bg: 'bg-cyan-500/10' },
            { label: 'Success Rate', value: `${successRate.toFixed(0)}%`, icon: BarChart3, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10' },
            { label: 'Total Txns', value: `${transactions.length}`, icon: Clock, color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10' },
          ].map((m) => (
            ...
            {transactions.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={150}>
                <BarChart data={dailyVolume.slice(-14)}>
                  ...
                  <Tooltip contentStyle={getTooltipStyle(isDark)} formatter={(v: number) => [v, 'Transactions']} />
        ...
        <motion.div
          ...
          className="glass-card p-6 border-dashed border-black/[0.1] dark:border-white/[0.1]"
        >
          <div className="text-center py-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-600 dark:text-violet-400 text-xs mb-3">
              Phase 2 — Coming Soon
            </div>

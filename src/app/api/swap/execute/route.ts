...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!VALID_TOKENS.includes(fromToken) || !VALID_TOKENS.includes(toToken) || fromToken === toToken) {
      return NextResponse.json({ error: 'Unsupported or invalid swap pair' }, { status: 400 });
    }
    ...
    // insufficient-reserve executions from entering the dispatch path.
    try {
    ...
    if (!dispatch.success) {
      ...
      return NextResponse.json({ error: `Swap settlement failed: ${dispatch.reason}. Your ${fromToken} deposit is recorded against tx ${inboundTxHash}.` }, { status: 500 });
    }

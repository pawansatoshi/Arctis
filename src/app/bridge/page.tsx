          ...
          <div className="space-y-2">
            {burnTxHash && selectedRoute && (
              <a href={`${selectedRoute.explorer}/tx/${burnTxHash}`} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-between glass-card p-3 hover:bg-white/[0.06] transition-colors">
                <span className="text-surface-600 text-xs">Source tx ({selectedRoute.sourceChain})</span>
                <span className="text-blue-400 text-xs flex items-center gap-1">View <ExternalLink className="w-3 h-3" /></span>
              </a>
            )}
            {forwardTxHash && (
              <a href={txUrl(forwardTxHash)} target="_blank" rel="noopener noreferrer"
                ...
                <span className="text-blue-400 text-xs flex items-center gap-1">ArcScan <ExternalLink className="w-3 h-3" /></span>
              </a>
            )}
          </div>
  ...
  return (
    <div className="page-container max-w-lg">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-surface-950">Bridge</h1>
          <p className="text-surface-600 text-sm mt-1">CCTP V2 · Bring USDC to Arc Testnet</p>
        </div>
        <button onClick={() => setShowHistory((s: boolean) => !s)} className={cn('btn-ghost', showHistory && 'bg-white/[0.08]')}>
      ...
      {!isConnected ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          <p className="text-surface-600 text-sm">Bridge History</p>
          {history.length === 0 ? (
            <div className="glass-card p-6 text-center text-surface-600 text-sm">No bridges yet</div>
            ...
            <div key={b.burnTxHash} className="glass-card p-4 flex items-center justify-between">
              ...
              <span className={cn('text-xs px-2 py-0.5 rounded-full',
                ...
                b.status === 'failed' ? 'bg-rose-500/10 text-rose-400' :
                b.status === 'timeout' ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400')}>
                {b.status}
              </span>
            </div>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <div className="glass-card p-4 space-y-3">
            ...
            <div className="relative">
              <button onClick={() => setShowChainMenu((s: boolean) => !s)} disabled={step !== 'idle'}
                className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] transition-colors disabled:opacity-60">
                  ...
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    {routes.map((r: BridgeRoute) => (
                      <button key={r.sourceChainId} onClick={() => { setSelectedRoute(r); setShowChainMenu(false); setAmount(''); setQuote(null); }}
                        className={cn('w-full px-4 py-3 text-left hover:bg-white/[0.06] transition-colors text-sm text-surface-950',
                          r.sourceChainId === selectedRoute?.sourceChainId && 'bg-white/[0.06]')}>
                        {r.sourceChain}
                      </button>
                    ))}
                  </motion.div>
          ...
          <div className="glass-card p-4 space-y-2">
            <label className="text-surface-600 text-xs uppercase tracking-wider">Amount (USDC)</label>
          ...
          {quote && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-4 space-y-2 text-sm">
              ...
              {quote.feeEstimated && (
                <p className="text-amber-400 text-xs flex items-center gap-1.5 pt-1"><Info className="w-3 h-3" /> Estimated fee — live quote unavailable</p>
              )}
            </motion.div>
          )}
          ...
          {step === 'attesting' && (
            <div className="glass-card p-5 text-center space-y-3 border-blue-500/20 bg-blue-500/5">
              ...
              {burnTxHash && selectedRoute && (
                <a href={`${selectedRoute.explorer}/tx/${burnTxHash}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-xs inline-flex items-center gap-1">
          ...
          {step === 'approved' && <button onClick={handleBurn} className="btn-primary w-full justify-center bg-emerald-500 hover:bg-emerald-400"><CheckCircle2 className="w-4 h-4" /> Confirm Bridge</button>}

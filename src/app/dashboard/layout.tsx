...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const { sidebarCollapsed, toggleSidebar, creditBalance } = useAppStore();
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      {/* Logo */}
      <div className={cn(
        'flex items-center gap-3 border-b border-white/[0.06] flex-shrink-0 transition-all duration-300',
        sidebarCollapsed ? 'px-3 py-4 justify-center' : 'px-4 py-4',
      )}>
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500/30 to-violet-500/20 border border-white/[0.1] flex items-center justify-center flex-shrink-0 shadow-lg shadow-blue-500/10">
      ...
      {isConnected && !isCorrectChain && (
        <button
          onClick={switchToArc}
          className={cn(
            'm-2.5 rounded-xl bg-amber-500/10 border border-amber-500/25 text-amber-400 text-xs hover:bg-amber-500/15 transition-colors',
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      'flex items-center gap-3 rounded-xl text-sm font-medium transition-all duration-150 relative',
                      sidebarCollapsed ? 'px-2 py-2 justify-center' : 'px-3 py-2',
                      active
                        ? 'bg-blue-500/12 text-blue-400 border border-blue-500/20'
                        : 'text-surface-600 hover:text-surface-950 hover:bg-white/[0.04]',
                    )}
                  >
        ...
        <div className="mx-3 mb-3 p-3.5 glass-card space-y-2.5">
          ...
          {creditBalance && (
            <div className="flex items-center justify-between pt-2 border-t border-white/[0.06]">
              ...
              <Link href="/credits" className="text-blue-400 text-[10px] hover:text-blue-300 transition-colors">
                Top up →
              </Link>
            </div>
          )}
      ...
      <button
        ...
        className="hidden md:flex items-center justify-center h-10 flex-shrink-0 border-t border-white/[0.06] text-surface-600 hover:text-surface-950 hover:bg-white/[0.04] transition-all"
      >
  ...
  return (
    <div className="min-h-screen bg-surface-50 flex">
      ...
      {/* ── Desktop sidebar ──────────────────────────────────── */}
      <aside className={cn(
        'hidden md:flex fixed left-0 top-0 h-full z-40 flex-col bg-surface-100 border-r border-white/[0.06] transition-all duration-300',
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          />
          <aside className="relative z-10 w-[240px] h-full flex flex-col bg-surface-100 border-r border-white/[0.06] shadow-2xl">
            <button
              onClick={() => setMobileOpen(false)}
              aria-label="Close menu"
              className="absolute top-4 right-3 p-1.5 rounded-lg text-surface-600 hover:text-surface-950 hover:bg-white/[0.06] transition-all"
            >
              <X className="w-4 h-4" />
            </button>
            <SidebarContent />
      ...
      <div className={cn(
        ...
        {/* Header */}
        <header className="sticky top-0 z-30 flex items-center gap-4 px-4 sm:px-6 h-14 bg-surface-50/90 backdrop-blur-xl border-b border-white/[0.06]">

          {/* Mobile menu trigger */}
          <button
            onClick={() => setMobileOpen(true)}
            aria-label="Open navigation"
            className="md:hidden p-2 -ml-1 rounded-xl text-surface-600 hover:text-surface-950 hover:bg-white/[0.06] transition-all"
          >
            ...
            {isConnected && (
              <div className={cn(
                ...
                isCorrectChain
                  ...
                  : 'bg-amber-500/10 text-amber-400 border border-amber-500/20 cursor-pointer hover:bg-amber-500/15',
              )}

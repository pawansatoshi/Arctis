...
function TxTypePill({ type }: { type?: string }) {
  const map: Record<string, string> = {
    ...
    swap:            'bg-violet-500/10 text-violet-400',
    bridge:          'bg-cyan-500/10 text-cyan-400',
    credit_purchase: 'bg-amber-500/10 text-amber-400',
    membership:      'bg-emerald-500/10 text-emerald-400',
  };
...
/* ── OS Pillars data ───────────────────────────────────────── */
const PILLARS = [
  { id: 'ai',      label: 'AI OS',           sub: 'Copilot & Chat',    href: '/ai',       icon: Bot,          from: 'from-violet-500/20', border: 'border-violet-500/20', iconColor: 'text-violet-400', dot: 'bg-violet-400' },
  { id: 'swap',    label: 'Stablecoin OS',   sub: 'Transfer · Swap · Bridge', href: '/swap', icon: ArrowLeftRight, from: 'from-blue-500/20',   border: 'border-blue-500/20',   iconColor: 'text-blue-400',   dot: 'bg-blue-400' },
  { id: 'agents',  label: 'Economic Agents', sub: 'Approve & Deploy',  href: '/agents',   icon: GitMerge,     from: 'from-emerald-500/20',border: 'border-emerald-500/20',iconColor: 'text-emerald-400',dot: 'bg-emerald-400' },
  { id: 'knowledge', label: 'Knowledge OS', sub: 'Sessions & Prompts', href: '/workspace',icon: Sparkles,     from: 'from-amber-500/20',  border: 'border-amber-500/20',  iconColor: 'text-amber-400',  dot: 'bg-amber-400' },
] as const;

/* ── Quick actions ─────────────────────────────────────────── */
const QUICK = [
  { label: 'AI Workspace', href: '/ai',       icon: Bot,           color: 'text-violet-400', bg: 'bg-violet-500/10' },
  { label: 'Swap',         href: '/swap',     icon: ArrowLeftRight, color: 'text-blue-400',  bg: 'bg-blue-500/10'  },
  { label: 'Agents',       href: '/agents',   icon: GitMerge,      color: 'text-emerald-400',bg: 'bg-emerald-500/10'},
  { label: 'Treasury',     href: '/treasury', icon: Building2,     color: 'text-amber-400',  bg: 'bg-amber-500/10' },
] as const;
...
export default function DashboardPage() {
    ...
    return (
      ...
      >
        ...
        <div className="relative">
          <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-blue-500/20 to-violet-500/20 border border-white/[0.08] flex items-center justify-center mx-auto mb-6 shadow-xl">
  ...
  /* ── Connected ───────────────────────────────────────────── */
  return (
    <motion.div variants={page} initial="hidden" animate="show" className="max-w-5xl space-y-5 pb-10">

      {/* ── HEADER ─────────────────────────────────────────── */}
      <motion.div variants={row} className="flex items-start justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            ...
            className="p-2.5 rounded-xl text-surface-600 hover:text-surface-950 hover:bg-white/[0.07] transition-all duration-200 active:scale-95"
          >
        ...
        {actionItems.length > 0 && (
          ...
          >
            ...
            {actionItems.map((a, i) => {
              const Icon = a.icon;
              const toneMap = {
                amber: { card: 'border-amber-500/25 bg-amber-500/[0.06]', icon: 'bg-amber-500/15 border-amber-500/20', iconColor: 'text-amber-400', cta: 'text-amber-400 hover:text-amber-300 hover:bg-amber-500/10' },
                rose:  { card: 'border-rose-500/25 bg-rose-500/[0.06]',   icon: 'bg-rose-500/15 border-rose-500/20',   iconColor: 'text-rose-400',   cta: 'text-rose-400 hover:text-rose-300 hover:bg-rose-500/10' },
                blue:  { card: 'border-blue-500/25 bg-blue-500/[0.06]',   icon: 'bg-blue-500/15 border-blue-500/20',   iconColor: 'text-blue-400',   cta: 'text-blue-400 hover:text-blue-300 hover:bg-blue-500/10' },
              };
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {QUICK.map((q) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <span className="text-surface-950 text-sm font-medium group-hover:text-blue-400 transition-colors duration-200">
                  {q.label}
                </span>
                <ArrowUpRight className="w-3.5 h-3.5 text-surface-600 group-hover:text-blue-400 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all duration-200" />
              </div>
            </Link>
          ))}
        </div>
            ...
            <div className="divide-y divide-white/[0.035]">
              {recentTxs.map((tx: TransactionRecord, i: number) => (
                <motion.div
                  ...
                  className="flex items-center gap-4 px-6 py-3.5 hover:bg-white/[0.025] transition-colors duration-150 group"
                >
                  {/* Status icon */}
                  <div className="w-8 h-8 rounded-full bg-surface-300/50 border border-white/[0.05] flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {tx.txHash && (
                        <a
                          ...
                          className="text-blue-400/70 text-[10px] hover:text-blue-400 transition-colors font-mono"
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className={cn(
                      ...
                      tx.status === 'failed' ? 'text-rose-400' : 'text-surface-950',
                    )}>
                      -{tx.amountFormatted}
                    </div>

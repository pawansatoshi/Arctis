...
const CATEGORIES = [
  { id: 'bug_report',      label: 'Bug Report',       icon: Bug,           color: 'text-rose-400',   bg: 'bg-rose-500/10',   border: 'border-rose-500/20'   },
  { id: 'feature_request', label: 'Feature Request',  icon: Lightbulb,     color: 'text-amber-400',  bg: 'bg-amber-500/10',  border: 'border-amber-500/20'  },
  { id: 'improvement',     label: 'Improvement',      icon: TrendingUp,    color: 'text-blue-400',   bg: 'bg-blue-500/10',   border: 'border-blue-500/20'   },
  { id: 'general',         label: 'General',          icon: MessageSquare, color: 'text-violet-400', bg: 'bg-violet-500/10', border: 'border-violet-500/20' },
] as const;
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      >
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? `${cat.border} ${cat.bg}`
                      : 'border-white/[0.06] hover:bg-white/[0.04] hover:border-white/[0.10]'
                  )}
                >
                  <div className={cn(
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-1.5">
            {form.message.trim().length > 0 && form.message.trim().length < 10 && (
              <p className="text-rose-400 text-xs">At least 10 characters required</p>
            )}
        ...
        {address && (
          <div className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-surface-300/30 border border-white/[0.05]">
      ...
      >
        <Heart className="w-4 h-4 text-rose-400 flex-shrink-0 mt-0.5" aria-hidden />

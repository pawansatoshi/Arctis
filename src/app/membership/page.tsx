...
const TIER_STYLES: Record<MembershipTier, { border: string; badge: string; highlight: boolean }> = {
  free:       { border: 'border-white/[0.06]',   badge: 'bg-surface-300/50 text-surface-600', highlight: false },
  student:    { border: 'border-emerald-500/20', badge: 'bg-emerald-500/10 text-emerald-400', highlight: false },
  pro:        { border: 'border-blue-500/30',    badge: 'bg-blue-500/20 text-blue-400',       highlight: true  },
  enterprise: { border: 'border-violet-500/20', badge: 'bg-violet-500/10 text-violet-400',   highlight: false },
};
        ...
        {MEMBERSHIP_PLANS.map((plan) => {
          ...
          return (
            <div key={plan.id} className={cn(
              ...
              {isCurrent ? (
                <div className="w-full py-2 rounded-xl text-center text-sm text-surface-600 border border-white/[0.06]">
                  Current plan
                </div>
              ) : plan.priceUSDC === 0 ? (
                <div className="w-full py-2 rounded-xl text-center text-sm text-surface-600 border border-white/[0.06]">

...
import {
  ...
  ExternalLink, Copy, Check, ChevronRight, Building2, BarChart3,
} from 'lucide-react';
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        {isConnected && address && (
          <motion.div
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-sm"
          >
...
function StepCredits({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-card p-4 max-w-sm mx-auto text-center">
        <p className="text-surface-600 text-xs uppercase tracking-wide font-medium mb-2">How Credits Work</p>
        <p className="text-surface-700 text-sm leading-relaxed">
          ARCTIS AI is a flat <span className="text-surface-950 font-mono font-medium">1 credit / 1,000 tokens</span> for every
          conversation — simple, predictable, no surprises. ARCTIS Copilot is always free.
        </p>
      </div>
...
function StepExplore({ onFinish }: { onFinish: () => void }) {
  const features = [
    { icon: Bot,          label: 'AI Workspace',    href: '/ai',        desc: '12 specialized AI modes',        color: 'text-blue-600 dark:text-blue-400',   bg: 'bg-blue-500/10' },
    { icon: Zap,          label: 'Economic Agents', href: '/agents',    desc: 'Deploy autonomous worker agents', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/10' },
    { icon: Building2,    label: 'Treasury',        href: '/treasury',  desc: 'Track USDC flows and revenue',    color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10' },
    { icon: BarChart3,    label: 'Analytics',       href: '/analytics', desc: 'Charts and insights',             color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto">
        {features.map((f) => (
          ...
          >
            <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center mb-2.5', f.bg)}>
...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-50 flex items-center justify-center px-4 safe-bottom">
      {/* Background */}

    ...
    <div className="min-h-screen bg-surface-50 overflow-x-hidden">
      ...
      <section className="relative z-10 pt-12 sm:pt-20 pb-16 px-4 sm:px-6 text-center max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-medium mb-8"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />Built on Arc · {NETWORK_NAME} · Chain {CHAIN_ID}</div>
      ...
      <section className="relative z-10 py-20 px-4 sm:px-6 text-center"><motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-2xl mx-auto"><h2 className="text-3xl sm:text-4xl font-bold text-surface-950 mb-5 tracking-tight">Ready to use programmable money?</h2><p className="text-surface-600 mb-8 text-base leading-relaxed">Connect your wallet, review every action, and use {NETWORK_NAME} with the existing ARCTIS transaction rails.</p><div className="flex flex-col sm:flex-row items-center justify-center gap-4"><Link href="/dashboard" className="btn-primary w-full sm:w-auto px-8 py-3.5 text-base shadow-xl shadow-blue-500/20">Launch ARCTIS <ChevronRight className="w-5 h-5" /></Link><Link href="/agents" className="btn-ghost w-full sm:w-auto px-7 py-3.5 text-base border border-black/[0.08] dark:border-white/[0.08]">Explore Agents</Link></div></motion.div></section>

...
export default function PassportPage() {
  ...
  if (loadingExisting) {
    return (
      <div className="page-container max-w-sm flex items-center justify-center min-h-[60vh]">
        <div className="glass-card p-8 flex items-center justify-center w-full">
          ...
          {editing && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }} className="mt-4 glass-card p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setEditing(false)} aria-label="Cancel editing" className="btn-ghost px-2 py-1"><XIcon className="w-4 h-4" /></button>
              </div>
    ...
    <div className="page-container max-w-sm safe-bottom">
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        <div className="glass-card p-5 space-y-2.5">
          <label className="text-surface-600 text-xs font-medium uppercase tracking-wider">Username</label>
          ...
          {username.length > 0 && (
            <div className="flex items-center gap-1.5 text-xs">
              ...
              {availability === 'available' && <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />}
              {(availability === 'taken' || availability === 'invalid') && <XCircle className="w-3 h-3 text-rose-600 dark:text-rose-400" />}
              <span className={cn(availability === 'available' && 'text-emerald-600 dark:text-emerald-400', (availability === 'taken' || availability === 'invalid') && 'text-rose-600 dark:text-rose-400', availability === 'checking' && 'text-surface-500')}>
        ...
        <div className="glass-card p-5 space-y-2.5">
          <label className="text-surface-600 text-xs font-medium uppercase tracking-wider">Display Name <span className="normal-case text-surface-500">(optional)</span></label>

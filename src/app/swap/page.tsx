    ...
    return (
      <div className="page-container max-w-lg flex items-center justify-center min-h-[60vh]">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-8 text-center space-y-6 w-full">
          <div className="w-16 h-16 rounded-full bg-emerald-500/15 flex items-center justify-center mx-auto"><CheckCircle2 className="w-8 h-8 text-emerald-400" /></div>
          ...
          <div className="space-y-2">
            {inboundTxHash && <a href={txUrl(inboundTxHash)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between glass-card p-3 hover:bg-white/[0.06] transition-colors"><span className="text-surface-600 text-xs">Inbound tx</span><span className="text-blue-400 text-xs flex items-center gap-1">View <ExternalLink className="w-3 h-3" /></span></a>}
            {outboundTxHash && <a href={txUrl(outboundTxHash)} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between glass-card p-3 hover:bg-white/[0.06] transition-colors"><span className="text-surface-600 text-xs">Outbound tx</span><span className="text-blue-400 text-xs flex items-center gap-1">ArcScan <ExternalLink className="w-3 h-3" /></span></a>}
          </div>
  ...
  return (
    <div className="page-container max-w-lg">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-surface-950">Swap</h1><p className="text-surface-600 text-sm mt-1">OTC settlement · USDC ↔ tUSDC ↔ tARC</p></div>
        <button onClick={() => setShowHistory((s: boolean) => !s)} className={cn('btn-ghost', showHistory && 'bg-white/[0.08]')}><History className="w-4 h-4" /></button>
      </div>

      {!isConnected ? (
        <div className="glass-card p-8 text-center"><Wallet className="w-8 h-8 text-surface-600 mx-auto mb-3" /><p className="text-surface-700">Connect your wallet to swap tokens</p></div>
      ) : showHistory ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          {history.length === 0 ? <div className="glass-card p-6 text-center text-surface-600 text-sm">No swaps yet</div> : history.map((s: SwapRecord) => (
            <div key={s.id} className="glass-card p-4 flex items-center justify-between">
              ...
              <span className={cn('text-xs px-2 py-0.5 rounded-full', s.status === 'completed' ? 'bg-emerald-500/10 text-emerald-400' : s.status === 'failed' ? 'bg-rose-500/10 text-rose-400' : 'bg-blue-500/10 text-blue-400')}>{s.status}</span>
            </div>
          ))}
        </motion.div>
      ) : (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <div className="glass-card p-4 space-y-2">
            <label className="text-surface-600 text-xs uppercase tracking-wider">From</label>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <button onClick={() => setShowFromMenu((s: boolean) => !s)} disabled={step !== 'idle'} className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] transition-colors disabled:opacity-60">
                ...
                <AnimatePresence>{showFromMenu && (
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute top-full left-0 right-0 mt-1 glass-card z-20 overflow-hidden">
                    {TOKENS.filter((t) => t !== toToken).map((t) => (<button key={t} onClick={() => { setFromToken(t); setShowFromMenu(false); }} className="w-full px-4 py-3 text-left hover:bg-white/[0.06] transition-colors text-sm text-surface-950">{t}</button>))}
                  </motion.div>
                )}</AnimatePresence>
              </div>
          ...
          <div className="flex justify-center"><button onClick={handleFlip} disabled={step !== 'idle'} className="btn-ghost p-2 rounded-full"><ArrowUpDown className="w-4 h-4" /></button></div>

          <div className="glass-card p-4 space-y-2">
            <label className="text-surface-600 text-xs uppercase tracking-wider">To</label>
            <div className="flex gap-2 items-center">
              <div className="relative flex-1">
                <button onClick={() => setShowToMenu((s: boolean) => !s)} disabled={step !== 'idle'} className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] transition-colors disabled:opacity-60">
                ...
                <AnimatePresence>{showToMenu && (
                  <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute top-full left-0 right-0 mt-1 glass-card z-20 overflow-hidden">
                    {TOKENS.filter((t) => t !== fromToken).map((t) => (<button key={t} onClick={() => { setToToken(t); setShowToMenu(false); }} className="w-full px-4 py-3 text-left hover:bg-white/[0.06] transition-colors text-sm text-surface-950">{t}</button>))}
                  </motion.div>
                )}</AnimatePresence>
              </div>
          ...
          {quoteError && (
            <div className="glass-card p-3 border-amber-500/20 bg-amber-500/5 flex items-start gap-2">
              <Info className="w-4 h-4 text-amber-400 flex-shrink-0 mt-0.5" /><p className="text-amber-400 text-sm">{quoteError}</p>
            </div>
          )}
          ...
          {step === 'error' && errorMsg && <div className="glass-card p-3 border-rose-500/20 bg-rose-500/5 flex items-start gap-2"><AlertCircle className="w-4 h-4 text-rose-400 flex-shrink-0 mt-0.5" /><p className="text-rose-400 text-sm">{errorMsg}</p></div>}

...
import { useArcMemo } from '@/lib/memo/useMemo';
...
function isOtcPair(fromToken: SwapToken, toToken: SwapToken) {
  return !isCircleSwapToken(fromToken) || !isCircleSwapToken(toToken);
}
...
function SwapPageInner() {
    ...
    try {
      if (circlePair) {
        ...
        const estimate = await kit.estimateSwap({
          ...
          tokenIn: fromToken,
          tokenOut: toToken,
          amountIn: amountNum.toString(),
          config: { slippageBps: 100 },
        });
        const estimatedOutput = Number(estimate.estimatedOutput.amount);
        const stopLimit = estimate.stopLimit.amount;
        setQuote({ rail: 'circle', inputAmount: amountNum, outputAmount: estimatedOutput, rate: estimatedOutput / amountNum, fee: feeFromSwapEstimate(estimate.fees), feeLabel: 'Circle provider / kit fees', routeAvailable: true, stopLimit });
      } else if (otcPair) {
      ...
      setQuote(null);
      setQuoteError(error instanceof Error ? error.message : 'Unable to get a live swap quote.');
      setStep('idle');
    }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        void dispatchMemo({ type: 'swap', swapId: inboundTxHash, routeId: quote.rail === 'otc' ? `${fromToken.toLowerCase()}-${toToken.toLowerCase()}` : 'circle', fromToken, toToken });
  ...
  }, [inboundConfirmed, step, inboundTxHash, address, quote, fromToken, toToken, amountNum, getAuthHeaders, saveLocal, dispatchMemo, amount]);
  ...
  const executeSwap = useCallback(async () => {
    ...
    try {
      ...
      if (quote.rail === 'circle') {
        ...
        const stopLimit = estimate.stopLimit.amount;
        const result = await kit.swap({ from: { adapter, chain: 'Arc_Testnet' }, tokenIn: fromToken, tokenOut: toToken, amountIn: amountNum.toString(), config: { slippageBps: 100, stopLimit } });
        ...
        void dispatchMemo({ type: 'swap', swapId: txHash, routeId: `circle-${fromToken.toLowerCase()}-${toToken.toLowerCase()}`, fromToken, toToken });
      ...
      setErrorMsg(error instanceof Error ? error.message : 'Swap failed.');
      setStep('error');
    }
  }, [isConnected, address, connector, quote, amountValid, chainId, switchChainAsync, fromToken, toToken, amountNum, amount, writeContractAsync, saveLocal, dispatchMemo]);
  ...
  const executeAgentSwap = useCallback(async (proposal: import('@/lib/store').PendingFinancialAction) => {
    ...
    if (from === to || (!isCircleSwapPair(from, to) && !(isOtcPair(from, to) && !isCircleSwapPair(from, to)))) throw new Error('This swap pair is not supported.');
  ...
  const reset = () => { setStep('idle'); setAmount(''); setQuote(null); setQuoteError(null); setErrorMsg(null); setInboundTxHash(undefined); setOutboundTxHash(null); setAgentExecuting(false); };
  ...
  return (
    <div className="page-container max-w-lg safe-bottom">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-7">
        <div><span className="text-surface-500 text-xs font-semibold uppercase tracking-widest">Stablecoin OS</span><h1 className="text-2xl font-bold text-surface-950 tracking-tight mt-1">Swap</h1><p className="text-surface-600 text-sm mt-1">Circle liquidity + ARCTIS OTC settlement</p></div>
        <button onClick={() => setShowHistory((v) => !v)} aria-label="Swap history" className={cn('btn-ghost', showHistory && 'bg-blue-500/10 text-blue-600')}><History className="w-4 h-4" /></button>
      </motion.div>
          ...
          {isConnected && (
            <>
              <div className="glass-card p-5 space-y-3">
                <div className="relative"><label className="text-surface-600 text-xs font-medium uppercase tracking-wider">From</label><button onClick={() => setShowFromMenu((v) => !v)} className="mt-1 w-full flex items-center justify-between px-4 py-3 rounded-xl bg-black/[.04] dark:bg-white/[.04] border border-black/[.08] dark:border-white/[.08]"><span className="font-medium text-surface-950">{fromToken}</span><ChevronDown className="w-4 h-4 text-surface-500" /></button>{showFromMenu && <div className="absolute left-0 right-0 top-full mt-1 z-30 glass-card overflow-hidden">{TOKENS.filter((t) => t !== toToken).map((t) => <button key={t} onClick={() => { setFromToken(t); setShowFromMenu(false); }} className="w-full text-left px-4 py-3 text-sm hover:bg-black/[.05] dark:hover:bg-white/[.05]">{t}{isCircleSwapToken(t) ? ' · Circle' : ' · ARCTIS OTC'}</button>)}</div>}</div>
                <input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" min="0" step="0.000001" placeholder="0.00" className="input-field w-full" />
              </div>

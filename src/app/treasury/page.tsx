...
const TOOLTIP_STYLE = {
  backgroundColor: '#1c1c1f', border: '1px solid rgba(255,255,255,0.08)',
  borderRadius: '12px', color: '#f4f4f5', fontSize: '12px',
};
        ...
        {transactions.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={cashFlow}>
              ...
              <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number) => [`${v} USDC`, 'Outflow']} />
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-3">
            {[
              { label: 'Confirmed', count: confirmedTxCount, icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
              { label: 'Pending', count: transactions.filter((t) => t.status === 'pending').length, icon: Clock, color: 'text-amber-400', bg: 'bg-amber-500/10' },
              { label: 'Failed', count: failedTxCount, icon: XCircle, color: 'text-rose-400', bg: 'bg-rose-500/10' },
            ].map((s) => (
          ...
          {logs.length === 0 ? (
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {logs.slice(0, 20).map((log) => (
                <div key={log.id} className="flex items-center justify-between py-2 border-b border-white/[0.04]">
                  <div>
                  ...
                  <span className={cn('font-mono text-sm font-bold',
                    log.type === 'ai_spend' ? 'text-rose-400' : 'text-emerald-400'
                  )}>

...
import { savePrompt, getUserPrompts, deletePrompt } from '@/lib/api/prompts-client';
...
const WORKSPACE_DOMAINS: Record<WorkspaceDomain, {
  ...
  student: {
    icon: GraduationCap, label: 'Student', color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10', mode: 'student',
  ...
  teacher: {
    icon: Users, label: 'Teacher', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10', mode: 'teacher',
  ...
  professor: {
    icon: FlaskConical, label: 'Professor', color: 'text-cyan-600 dark:text-cyan-400', bg: 'bg-cyan-500/10', mode: 'professor',
  ...
  research: {
    icon: Search, label: 'Research', color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10', mode: 'research',
  ...
  developer: {
    icon: Code2, label: 'Developer', color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10', mode: 'developer',
  ...
  engineering: {
    icon: Wrench, label: 'Engineering', color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/10', mode: 'engineering',
  ...
  treasury: {
    icon: TrendingUp, label: 'Treasury', color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10', mode: 'treasury',
  ...
  child: {
    icon: BookOpen, label: 'Kids', color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-500/10', mode: 'child',
    ...
    <div className="max-w-5xl space-y-6">
          ...
          <button key={key} onClick={() => setActiveDomain(key)} title={d.description}
            className={cn(
              ...
              activeDomain === key
                ...
                : 'border-black/[0.06] dark:border-white/[0.06] bg-surface-200/40 hover:bg-surface-200/80 hover:border-black/[0.1] dark:hover:border-white/[0.1]'
            )}>
            <div className={cn(
            ...
            <span className={cn('text-xs font-medium leading-tight',
              activeDomain === key ? 'text-blue-600 dark:text-blue-400' : 'text-surface-700'
            )}>
              {d.label}
            </span>
          </button>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={activeDomain}>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div className="flex gap-1 p-1 bg-surface-200/50 rounded-lg border border-black/[0.06] dark:border-white/[0.06]">
            {(['templates', 'library'] as const).map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={cn('px-3 py-1 rounded-md text-xs font-medium capitalize transition-all',
                  activeTab === tab ? 'bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'text-surface-600 hover:text-surface-950'
                )}>
              ...
              <div key={i} className="group glass-card-hover p-4 flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-200 flex-shrink-0 mt-0.5">
                  <button onClick={() => handleCopy(template, i)} aria-label="Copy template"
                    className="p-1.5 rounded-lg text-surface-500 hover:text-surface-950 hover:bg-black/[0.07] dark:hover:bg-white/[0.07] transition-all">
                    {copiedIdx === i ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                  <button onClick={() => handleSavePrompt(template)} aria-label="Save to library"
                    className="p-1.5 rounded-lg text-surface-500 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-500/10 transition-all">
                    <Star className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleUseTemplate(template)} aria-label="Use in AI Workspace"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/15 text-blue-600 dark:text-blue-400 hover:bg-blue-500/25 text-xs font-medium transition-all">
            ...
            {address && !loadingPrompts && savedPrompts.length === 0 && (
              <div className="glass-card p-10 text-center">
                <div className="w-12 h-12 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center mx-auto mb-4">
                  <Star className="w-5 h-5 text-amber-600 dark:text-amber-400 opacity-60" />
                </div>
              ...
              <div key={prompt.id} className="group glass-card-hover p-4 flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => handleUsePrompt(prompt)}
                    className="p-1.5 rounded-lg text-surface-600 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-500/10 transition-all">
                    <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleDeletePrompt(prompt.id)}
                    className="p-1.5 rounded-lg text-surface-600 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-500/10 opacity-0 group-hover:opacity-100 transition-all">

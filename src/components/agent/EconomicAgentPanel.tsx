...
function BridgeNetworkChoices({ missing, sourceChain, onSelect, disabled }: { missing?: PendingFinancialAction['missing']; sourceChain?: string; onSelect: (network: string) => void; disabled: boolean }) {
  ...
  return <div className="mt-3 rounded-xl border border-blue-500/15 bg-blue-500/[.04] p-3 text-left">
    <div className="flex items-center justify-between mb-2"><span className="text-[11px] font-semibold uppercase tracking-wider text-surface-500">{choosingSource ? 'Bridge from' : 'Bridge to'}</span><span className="text-[10px] text-surface-400">Tap to choose</span></div>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{options.map((network) => <button key={network} type="button" disabled={disabled} onClick={() => onSelect(network)} className="rounded-lg border border-black/[.06] dark:border-white/[.08] bg-surface-0 dark:bg-surface-200/70 px-3 py-2.5 text-left text-xs font-medium text-surface-800 dark:text-surface-100 hover:border-blue-500/40 hover:bg-blue-500/[.06] hover:text-blue-600 transition-colors disabled:opacity-50"><span>{network}</span></button>)}</div>
  </div>;
}

function TokenChoices({ onSelect, disabled }: { onSelect: (token: string) => void; disabled: boolean }) {
  return <div className="mt-3 grid grid-cols-2 sm:grid-cols-5 gap-2">{SWAP_TOKENS.map((token) => <button key={token} type="button" disabled={disabled} onClick={() => onSelect(token)} className="rounded-lg border border-black/[.06] dark:border-white/[.08] bg-surface-0 dark:bg-surface-200/70 px-2.5 py-2 text-xs font-semibold hover:border-violet-500/40 hover:text-violet-600 transition-colors disabled:opacity-50">{token}</button>)}</div>;
}
...
function QuickAmounts({ onSelect, disabled, max }: { onSelect: (amount: string) => void; disabled: boolean; max?: string }) {
  ...
  return <div className="flex flex-wrap gap-1.5 mt-2">{values.map((amount) => <button key={amount} type="button" disabled={disabled} onClick={() => onSelect(amount)} className="rounded-full border border-black/[.06] dark:border-white/[.08] px-3 py-1.5 text-[11px] font-medium hover:border-blue-500/40 hover:text-blue-600 disabled:opacity-50">{amount === max ? 'MAX' : amount} USDC</button>)}</div>;
}
...
function ProposalCard({ action, proposal, onApprove, onCancel, onReverse }: { action: LockedAction; proposal: PendingFinancialAction; onApprove: () => void; onCancel: () => void; onReverse?: () => void }) {
  ...
  return <div className="mt-3 rounded-2xl border border-amber-500/25 bg-amber-500/[0.07] p-4 space-y-3">
    <div className="flex items-center gap-2"><div className="w-7 h-7 rounded-lg bg-amber-500/15 flex items-center justify-center"><ShieldCheck className="w-4 h-4 text-amber-600" /></div><div><span className="text-amber-700 text-xs font-semibold uppercase tracking-wide">{PROPOSAL_TITLE[action]}</span><p className="text-[10px] text-surface-500">Nothing has been signed</p></div></div>
    ...
    <div className="flex items-center gap-2 text-[11px] text-surface-600"><Wallet className="w-3.5 h-3.5" /> Preflight checks balance, gas and route before wallet approval.</div>
...
export function EconomicAgentPanel({ action, onExecute, executionStatus = 'idle', executionError, executionTxHash }: { action: LockedAction; onExecute: (proposal: PendingFinancialAction) => void | Promise<void>; executionStatus?: AgentExecutionStatus; executionError?: string | null; executionTxHash?: string | null }) {
  ...
  useEffect(() => { if (action === 'swap') { setFromToken('USDC'); setToToken('EURC'); } }, [action]);
  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: generateId(), role: 'assistant', content: `I couldn't prepare that action: ${(err as Error).message}. Try one of the suggested options below.` }]);
  ...
  const handleReverse = (msgId: string) => {
    setMessages((prev) => prev.map((m) => {
      if (m.id !== msgId || !m.proposal?.toToken) return m;
      return { ...m, proposal: { ...m.proposal, fromToken: m.proposal.toToken, toToken: m.proposal.fromToken } };
    }));
  };
  ...
  return <div className="glass-card p-5 flex flex-col" style={{ minHeight: '460px' }}>
    ...
    <div className="flex-1 overflow-y-auto py-3 space-y-4 min-h-[220px]">
      {draftMode && messages.length === 0 && <div className="text-center py-4 px-1"><p className="text-surface-700 text-sm font-medium mb-3">{prompt}</p>
        ...
        <div className="mt-3 space-y-1.5">{examples.map((ex) => <button key={ex} onClick={() => send(ex)} className="block w-full text-left px-3 py-2 rounded-lg glass-card-hover text-surface-700 text-xs font-mono hover:text-blue-600">&quot;{ex}&quot;</button>)}</div>
      </div>}

      {messages.map((msg) => <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={cn('flex gap-2.5', msg.role === 'user' && 'flex-row-reverse')}><div className={cn('w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 border', msg.role === 'user' ? 'bg-blue-500/15 border-blue-500/20' : 'bg-violet-500/10 border-violet-500/20')}>{msg.role === 'user' ? <User className="w-3.5 h-3.5 text-blue-600" /> : <Bot className="w-3.5 h-3.5 text-violet-600" />}</div><div className={cn('flex-1 max-w-[88%]', msg.role === 'user' && 'flex flex-col items-end')}><div className={cn('px-3.5 py-2.5 text-sm leading-relaxed rounded-2xl', msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-md' : 'bg-surface-0 border border-black/[.06] text-surface-950 rounded-tl-md')}><div className="whitespace-pre-wrap break-words">{msg.content}</div>{msg.role === 'assistant' && msg.id === latestAssistantId && <BridgeNetworkChoices missing={pendingClarification?.missing} sourceChain={pendingClarification?.sourceChain} onSelect={handleNetworkChoice} disabled={isLoading} />}{msg.role === 'assistant' && msg.id === latestAssistantId && pendingClarification?.missing === 'toToken' && <TokenChoices onSelect={handleTokenChoice} disabled={isLoading} />}{msg.proposal && <ProposalCard action={action} proposal={msg.proposal} onApprove={() => onExecute(msg.proposal!)} onCancel={() => setMessages((prev) => prev.map((m) => m.id === msg.id ? { ...m, proposal: undefined } : m))} onReverse={action === 'swap' ? () => handleReverse(msg.id) : undefined} />}</div></div></motion.div>)}
    ...
    <div className="flex items-center gap-2 pt-2 border-t border-black/[.04]"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(input); } }} placeholder={`Optional: tell the agent what to ${action}…`} disabled={isLoading} className="input-base flex-1 text-sm" /><button onClick={() => send(input)} disabled={isLoading || !input.trim()} className="btn-primary px-3 py-2.5 disabled:opacity-50"><Send className="w-4 h-4" /></button></div>
  </div>;
}

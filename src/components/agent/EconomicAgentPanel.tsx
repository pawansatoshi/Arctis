...
import { Bot, User, Send, ShieldCheck, Sparkles, Loader2, Route, CircleDollarSign } from 'lucide-react';
...
export function EconomicAgentPanel({ action, onExecute, executionStatus = 'idle', executionError, executionTxHash }: { action: LockedAction; onExecute: (proposal: PendingFinancialAction) => void | Promise<void>; executionStatus?: AgentExecutionStatus; executionError?: string | null; executionTxHash?: string | null }) {
  ...
  return (
    <div className="glass-card p-5 flex flex-col" style={{ minHeight: '460px' }}>
      ...
      {executionStatus !== 'idle' && <div className="mb-3 rounded-xl border border-black/[.06] dark:border-white/[.07] bg-surface-0/60 dark:bg-surface-200/50 p-3"><div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-wider text-surface-500"><span className={cn('w-2 h-2 rounded-full', executionStatus === 'success' ? 'bg-emerald-500' : executionStatus === 'failed' ? 'bg-red-500' : 'bg-blue-500 animate-pulse')} />{executionStatus === 'executing' ? 'Running live checks / wallet approval' : executionStatus === 'success' ? 'Confirmed' : 'Execution failed'}</div>{executionTxHash && <p className="mt-1 text-[11px] font-mono text-surface-600 break-all">TX: {executionTxHash}</p>}{executionError && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{executionError}</p>}</div>}

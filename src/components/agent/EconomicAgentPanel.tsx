...
const INTRO_COPY: Record<LockedAction, { prompt: string; examples: string[] }> = {
  transfer: { prompt: 'Tell me who should receive the USDC and how much.', examples: ['Send 2 USDC to alice.arc', 'Send 5 USDC to 0x…', 'Send USDC to a Passport'] },
...
export function EconomicAgentPanel({ action, onExecute, executionStatus = 'idle', executionError, executionTxHash }: { action: LockedAction; onExecute: (proposal: PendingFinancialAction) => void | Promise<void>; executionStatus?: AgentExecutionStatus; executionError?: string | null; executionTxHash?: string | null }) {
  ...
  return <div className="glass-card p-5 flex flex-col" style={{ minHeight: '460px' }}>
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto py-4 space-y-4 min-h-[220px]">
      {messages.length === 0 && <div className="text-center py-6 px-2"><p className="text-surface-700 text-sm font-medium mb-3">{prompt}</p>{action === 'bridge' && <div className="mb-3 p-3 rounded-xl border border-blue-500/15 bg-blue-500/[.04] text-left"><label className="block text-[11px] font-semibold uppercase tracking-wider text-surface-500 mb-1.5">Bridge amount</label><div className="flex gap-2"><input value={bridgeAmount} onChange={(e) => setBridgeAmount(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); useBridgeAmount(); } }} type="number" min="0.000001" max="1000" step="0.000001" placeholder="0.00" className="input-base flex-1 text-sm" aria-label="Bridge amount in USDC" /><span className="self-center text-xs font-semibold text-surface-500">USDC</span><button onClick={useBridgeAmount} disabled={!bridgeAmount || Number(bridgeAmount) <= 0 || Number(bridgeAmount) > 1000 || isLoading} className="btn-primary px-3 py-2 disabled:opacity-50">Use</button></div><p className="mt-1.5 text-[10px] text-surface-500">Enter the amount first; the agent will ask for the source and destination if they are missing.</p></div>}<div className="space-y-1.5">{examples.map((ex) => <button key={ex} onClick={() => send(ex)} className="block w-full text-left px-3 py-2.5 rounded-lg glass-card-hover text-surface-700 text-xs font-mono hover:text-blue-600 dark:hover:text-blue-400 transition-colors">&quot;{ex}&quot;</button>)}</div></div>}

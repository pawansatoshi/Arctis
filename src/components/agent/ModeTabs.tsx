    ...
    try {
      ...
      if (kind === 'transfer') {
        ...
        const [remote, localRaw] = await Promise.all([
          ...
          Promise.resolve(typeof window !== 'undefined' ? localStorage.getItem(`arctis-swap-history:${address.toLowerCase()}`) : null),
        ]);
        ...
        const [remote, localRaw] = await Promise.all([
          fetch(`/api/bridge/history?wallet=${address}&limit=100`).then((r) => r.json()).catch(() => ({ bridges: [] })),
          Promise.resolve(typeof window !== 'undefined' ? localStorage.getItem(`arctis-bridge-history:${address.toLowerCase()}`) : null),
        ]);
        const local = localRaw ? JSON.parse(localRaw) as Array<Record<string, unknown>> : [];

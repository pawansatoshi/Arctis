  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full capitalize">{proposal.agentType}</span>
          </div>
          <p className="text-surface-500 text-[11px] mt-0.5 font-mono truncate">{proposal.model.split('/').pop()}</p>
        </div>
        <span className="text-[10px] text-blue-600 dark:text-blue-400 bg-blue-500/10 px-2 py-1 rounded-full whitespace-nowrap">Awaiting Review</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className={cn('rounded-xl p-2.5 text-center space-y-0.5', insufficientCredits ? 'bg-rose-500/10 border border-rose-500/20' : 'bg-black/[0.04] dark:bg-white/[0.04]')}>
          ...
          <p className={cn('text-sm font-bold', insufficientCredits ? 'text-rose-600 dark:text-rose-400' : 'text-surface-950')}>{proposal.estimatedCredits}</p>
        </div>
        <div className="rounded-xl p-2.5 text-center space-y-0.5 bg-black/[0.04] dark:bg-white/[0.04]">
        ...
        <div className={cn('rounded-xl p-2.5 text-center space-y-0.5', willExceedBudget ? 'bg-amber-500/10 border border-amber-500/20' : 'bg-black/[0.04] dark:bg-white/[0.04]')}>
          ...
          <p className={cn('text-sm font-bold', willExceedBudget ? 'text-amber-600 dark:text-amber-400' : 'text-surface-950')}>{isFinite(proposal.budgetRemaining) ? proposal.budgetRemaining : '∞'}</p>
        </div>
      </div>

      {insufficientCredits && (
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20">
          <AlertCircle className="w-4 h-4 text-rose-600 dark:text-rose-400 flex-shrink-0" /><p className="text-rose-600 dark:text-rose-400 text-xs">Insufficient credits — top up before approving</p>
        </div>
      )}
      {willExceedBudget && !insufficientCredits && (
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
          <AlertCircle className="w-4 h-4 text-amber-600 dark:text-amber-400 flex-shrink-0" /><p className="text-amber-600 dark:text-amber-400 text-xs">This will exceed the agent&apos;s monthly budget</p>
        </div>
      )}
        ...
        {!showRejectInput ? (
          <>
            <button onClick={() => setShowRejectInput(true)} disabled={!!deciding}
              className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium bg-black/[0.05] dark:bg-white/[0.05] hover:bg-rose-500/10 text-surface-600 hover:text-rose-600 dark:hover:text-rose-400 border border-black/[0.06] dark:border-white/[0.06] hover:border-rose-500/20 transition-all disabled:opacity-40">
            ...
            <button onClick={handleApprove} disabled={!canApprove || !!deciding}
              className={cn('flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-semibold transition-all',
                canApprove ? 'bg-emerald-500 hover:bg-emerald-400 text-white shadow-lg shadow-emerald-500/20' : 'bg-black/[0.05] dark:bg-white/[0.05] text-surface-500 cursor-not-allowed')}>
          ...
          <>
            <button onClick={() => { setShowRejectInput(false); setRejectReason(''); }} disabled={!!deciding}
              className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-black/[0.05] dark:bg-white/[0.05] text-surface-600 hover:bg-black/[0.08] dark:hover:bg-white/[0.08] transition-all disabled:opacity-40">
              Cancel
            </button>

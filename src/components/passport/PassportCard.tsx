...
function InitialsAvatar({ username }: { username: string }) {
  ...
  return (
    <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-white text-3xl font-bold shadow-lg ring-2 ring-black/10 dark:ring-white/10"
...
function QRPlaceholder({ username }: { username: string }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="w-28 h-28 rounded-xl border border-black/[0.08] dark:border-white/[0.08] bg-black/[0.03] dark:bg-white/[0.03] flex flex-col items-center justify-center gap-1.5">
        ...
        <div className="bg-surface-200 px-5 pt-12 pb-5 space-y-4">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              {isOwner && <span className="ml-auto text-[10px] bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-full font-medium">Your Passport</span>}
            </div>
          ...
          <div className="space-y-1.5">
            <button onClick={handleCopyAddress} className="w-full flex items-center justify-between bg-surface-300/50 hover:bg-surface-300 rounded-xl px-3.5 py-2.5 transition-colors group">
              ...
              {copiedAddr ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-surface-500 group-hover:text-surface-700 transition-colors" />}
            </button>
            <button onClick={handleCopyHandle} className="w-full flex items-center justify-between bg-surface-300/50 hover:bg-surface-300 rounded-xl px-3.5 py-2.5 transition-colors group">
              <div className="flex items-center gap-2"><Zap className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" /><span className="text-surface-700 text-xs font-mono">{username}.arc</span></div>
              {copiedHandle ? <Check className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-surface-500 group-hover:text-surface-700 transition-colors" />}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            <button onClick={onSend} className="flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 transition-all">

...
import {
  Search, LayoutDashboard, Activity, History, Bot, MessageCircle, Zap,
  FolderOpen, BookOpen, ArrowUpRight, ArrowLeftRight, GitMerge,
  Building2, Coins, Settings, Heart, CornerDownLeft,
} from 'lucide-react';
import { useAppStore } from '@/lib/store';

// ============================================================
// ARCTIS Command Palette — Ctrl+K / Cmd+K
// Everything accessible from one search, Apple/Linear-style.
// ============================================================
...
const ACTIONS: PaletteAction[] = [
  ...
  { id: 'activity',  label: 'Activity',     section: 'Overview',      href: '/activity',  icon: Activity },
  ...
  { id: 'copilot',   label: 'Copilot',      section: 'AI OS',         href: '/copilot',    icon: MessageCircle, keywords: 'help support docs' },
  { id: 'agents',    label: 'Agents',       section: 'AI OS',         href: '/agents',     icon: Zap, keywords: 'create agent new agent' },
  ...
  { id: 'transfer',  label: 'Transfer',     section: 'Stablecoin OS', href: '/transfer',   icon: ArrowUpRight, keywords: 'send usdc' },
  { id: 'swap',      label: 'Swap',         section: 'Stablecoin OS', href: '/swap',       icon: ArrowLeftRight, keywords: 'exchange tarc tusdc' },
  { id: 'bridge',    label: 'Bridge',       section: 'Stablecoin OS', href: '/bridge',     icon: GitMerge, keywords: 'cctp cross-chain' },
...
export default function CommandPalette() {
  ...
  // Global Ctrl/Cmd+K listener + Escape
  useEffect(() => {
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-[15vh] left-1/2 -translate-x-1/2 w-[92vw] max-w-lg z-[101]"
          >
            <div className="glass-card overflow-hidden shadow-2xl border border-black/[0.1] dark:border-white/[0.1]">
              <div className="flex items-center gap-3 px-4 py-3.5 border-b border-black/[0.06] dark:border-white/[0.06]">
                ...
                <input
                  ...
                  placeholder="Search ARCTIS — jump to any page…"
                  className="flex-1 bg-transparent outline-none text-surface-950 text-sm placeholder:text-surface-500"
                />
              ...
              <div className="max-h-[50vh] overflow-y-auto py-1.5">
                {filtered.length === 0 && (
                  <div className="px-4 py-8 text-center text-surface-500 text-sm">No matches</div>
                )}
                {filtered.map((action, i) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                      i === selected ? 'bg-blue-500/10' : 'hover:bg-black/[0.03] dark:hover:bg-white/[0.03]'
                    }`}
                  >
                    <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      i === selected ? 'bg-blue-500/20' : 'bg-surface-300/40'
                    }`}>

...
SUGGESTIONS: (one sentence on what to fix, or "None" if PASS)`;
  ...
  try {
    const result = await routeAIRequest({
      ...
      // No model hint — the router's health-based selection naturally
      // tends to pick a different model than the generator call most
      // of the time anyway, which is enough independence for this
      // adversarial review pass. (Previously hardcoded to a paid
      // Anthropic model, which contradicted the free-models-only design.)

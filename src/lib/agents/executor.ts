...
import { MEMBERSHIP_PLANS, OPERATION_COSTS } from '@/lib/memberships/plans';
...
export async function executeAgent(
  ...
  // ── Membership / model check ────────────────────────────
  const membership = await getMembership(agent.ownerWallet);
  const tier  = membership?.tier ?? 'free';
  const plan  = MEMBERSHIP_PLANS.find((p) => p.id === tier);
  const model = agent.model;

  if (plan && !plan.aiModels.includes('all')) {
    const modelShort = model.split('/').pop()?.split(':')[0] ?? '';
    const allowed = plan.aiModels.some((m) => model.includes(m) || m.includes(modelShort));
    if (!allowed) {
      throw new Error(`Model ${model} requires a higher membership tier (current: ${tier}).`);
    }
  }
...
export async function proposeAgent(params: ProposeAgentParams): Promise<AgentProposal> {
  ...
  const membership = await getMembership(owner);
  const tier = membership?.tier ?? 'free';
  const plan = MEMBERSHIP_PLANS.find((p) => p.id === tier);
  const model = agent.model;
  ...
  if (plan && !plan.aiModels.includes('all')) {
    const modelShort = model.split('/').pop()?.split(':')[0] ?? '';
    const allowed = plan.aiModels.some((m) => model.includes(m) || m.includes(modelShort));
    if (!allowed) throw new Error(`Model ${model} requires a higher membership tier (current: ${tier}).`);
  }
...
export async function approveProposal(
  ...
  const db = getDb();
  const snap = await getDoc(doc(db, 'agent_executions', proposalId));

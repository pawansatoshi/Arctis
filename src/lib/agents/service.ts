...
export async function createAgent(
  ...
  await db.collection(AGENTS_COL).doc(id).set({ ...agent, createdAt: FieldValue.serverTimestamp() });
  await writeAgentLedger(id, ownerWallet, {
    type: 'created', creditsAmount: 0, balanceBefore: 0, balanceAfter: 0,
...
export async function getUserAgents(ownerWallet: string): Promise<Agent[]> {
  const db = getAdminDb();
  // Keep the query on the stable ownerWallet+createdAt index and filter
  // archived agents in memory. This avoids requiring a fragile inequality
  // composite index involving status and __name__.
  ...
  return snap.docs
    .filter((d: QueryDocumentSnapshot) => d.data().status !== 'archived')
...
export async function checkAgentBudget(agent: Agent, estimatedCredits: number): Promise<{ ok: boolean; reason?: string }> {
  ...
  if (estimatedCredits > remaining) return { ok: false, reason: `Monthly budget exceeded. Remaining: ${remaining} credits, needed: ${estimatedCredits}` };
  if (estimatedCredits > agent.maxCreditsPerExecution) return { ok: false, reason: `Exceeds per-execution limit of ${agent.maxCreditsPerExecution} credits` };
  return { ok: true };
}

export async function recordExecution(execution: AgentExecution): Promise<void> {
  const db = getAdminDb();
  await db.collection(EXECUTIONS_COL).doc(execution.id).set({ ...execution, startedAt: FieldValue.serverTimestamp(), completedAt: execution.completedAt ? new Date(execution.completedAt) : null });
}

export async function updateExecution(executionId: string, updates: Partial<Pick<AgentExecution, 'status' | 'outputSummary' | 'outputFull' | 'creditsConsumed' | 'completedAt' | 'durationMs' | 'errorMessage' | 'reportId' | 'evaluationVerdict' | 'evaluationReasons' | 'evaluationSuggestions' | 'revisionCount'>>): Promise<void> {
  const db = getAdminDb();
...
export async function getAgentExecutions(agentId: string, limitCount = 20): Promise<AgentExecution[]> {
  const db = getAdminDb();
  const snap = await db.collection(EXECUTIONS_COL).where('agentId', '==', agentId).orderBy('startedAt', 'desc').limit(limitCount).get();
...
export async function getOwnerExecutions(ownerWallet: string, limitCount = 50): Promise<AgentExecution[]> {
  const db = getAdminDb();
  const snap = await db.collection(EXECUTIONS_COL).where('ownerWallet', '==', ownerWallet.toLowerCase()).orderBy('startedAt', 'desc').limit(limitCount).get();
  return snap.docs.map((d: QueryDocumentSnapshot) => ({ ...d.data(), id: d.id, startedAt: toIso(d.data().startedAt) }) as AgentExecution);
}

export async function chargeAgentExecution(agent: Agent, executionId: string, creditsConsumed: number): Promise<void> {
  const db = getAdminDb();
  await db.runTransaction(async (tx: Transaction) => {
    ...
    tx.update(agentRef, { creditsUsedThisMonth: newUsedThisMonth, totalCreditsConsumed: newTotal, executionCount: newCount, lastActiveAt: FieldValue.serverTimestamp(), status: 'idle' });
    const ledgerRef = db.collection(LEDGER_COL).doc();
    tx.set(ledgerRef, { agentId: agent.id, ownerWallet: agent.ownerWallet, type: 'execution', creditsAmount: -creditsConsumed, balanceBefore: agent.monthlyBudgetCredits - (current.creditsUsedThisMonth ?? 0), balanceAfter: agent.monthlyBudgetCredits - newUsedThisMonth, description: `Execution: ${agent.name}`, executionId, createdAt: FieldValue.serverTimestamp() });
  });
  await deductCredits(agent.ownerWallet, creditsConsumed, `Agent: ${agent.name} execution`, agent.model, executionId);
}

export async function saveAgentReport(report: AgentReport): Promise<void> {
  const db = getAdminDb();
  await db.collection(REPORTS_COL).doc(report.id).set({ ...report, createdAt: FieldValue.serverTimestamp() });
...
export async function getAgentReports(agentId: string): Promise<AgentReport[]> {
  const db = getAdminDb();
  const snap = await db.collection(REPORTS_COL).where('agentId', '==', agentId).orderBy('createdAt', 'desc').get();
...
export async function getOwnerReports(ownerWallet: string, limitCount = 30): Promise<AgentReport[]> {
  const db = getAdminDb();
  const snap = await db.collection(REPORTS_COL).where('ownerWallet', '==', ownerWallet.toLowerCase()).orderBy('createdAt', 'desc').limit(limitCount).get();
  return snap.docs.map((d: QueryDocumentSnapshot) => ({ ...d.data(), id: d.id, createdAt: toIso(d.data().createdAt) }) as AgentReport);
}

async function writeAgentLedger(agentId: string, ownerWallet: string, entry: Omit<AgentLedgerEntry, 'id' | 'agentId' | 'ownerWallet' | 'createdAt'>): Promise<void> {
  const db = getAdminDb();
  await db.collection(LEDGER_COL).doc().set({ agentId, ownerWallet: ownerWallet.toLowerCase(), ...entry, createdAt: FieldValue.serverTimestamp() });
}

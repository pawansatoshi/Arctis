...
function matchBridgeChain(text: string): BridgeChainMatch | null {
  ...
  for (const chain of BRIDGE_CHAIN_LIST) if (lower.includes(chain.name.toLowerCase())) return chain;
  const SHORTHANDS: Record<string, string> = { arc: 'Arc Testnet', ethereum: 'Ethereum Sepolia', eth: 'Ethereum Sepolia', sepolia: 'Ethereum Sepolia', base: 'Base Sepolia', arbitrum: 'Arbitrum Sepolia', arb: 'Arbitrum Sepolia' };
  for (const [short, full] of Object.entries(SHORTHANDS)) if (new RegExp(`\\b${short}\\b`, 'i').test(lower)) return BRIDGE_CHAIN_LIST.find((c) => c.name === full) ?? null;
  return null;
}
...
export function parseFinancialIntent(message: string): PendingFinancialAction | null {
  ...
  if (swapSelection) {
    const fromToken = normalizeSwapToken(swapSelection[1]);
    const toToken = normalizeSwapToken(swapSelection[2]);
    if (fromToken && toToken && isSupportedSwapPair(fromToken, toToken)) return { action: 'swap', amount: '', fromToken, toToken, missing: 'amount', createdAt: Date.now() };
  }

  const bridgeSelection = text.match(/\bbridge\s+usdc\s+from\s+(.+?)\s+to\s+(.+?)\s*$/i);
  if (bridgeSelection) {
    const source = matchBridgeChain(bridgeSelection[1]); const destination = matchBridgeChain(bridgeSelection[2]);
    if (source && destination && source.chainId !== destination.chainId) return { action: 'bridge', amount: '', fromToken: 'USDC', sourceChain: source.name, sourceChainId: source.chainId, destinationChain: destination.name, destinationChainId: destination.chainId, missing: 'amount', createdAt: Date.now() };
  }
  ...
  if (swapPartial) {
    const fromToken = normalizeSwapToken(swapPartial[2]);
  ...
  if (bridge) {
    ...
    if (!destination) return { ...base, sourceChain: source.name, sourceChainId: source.chainId, missing: 'sourceChain', error: 'Please specify a supported destination chain.' };
    if (source.chainId === destination.chainId) return { ...base, sourceChain: source.name, sourceChainId: source.chainId, missing: 'sourceChain', error: 'Source and destination chains must be different.' };
...
export function resolveClarification(pending: PendingFinancialAction, replyText: string): PendingFinancialAction {
  ...
  switch (pending.missing) {
    ...
    case 'recipient': { const address = text.match(new RegExp(`^${ADDRESS_RE}$`, 'i')); if (address) return { ...clean, recipient: address[0], missing: undefined }; const passport = text.match(new RegExp(`^${PASSPORT_RE}$`, 'i')); if (passport) return { ...clean, recipient: normalizePassportRecipient(passport[0]), missing: undefined }; return { ...clean, missing: 'recipient', error: 'Please provide a valid 0x wallet address or Passport username.' }; }
    case 'toToken': { const toToken = normalizeSwapToken(text.match(new RegExp(TOKEN_RE, 'i'))?.[0]); if (toToken && toToken !== clean.fromToken && isSupportedSwapPair(clean.fromToken, toToken)) return { ...clean, toToken, missing: undefined, error: undefined }; return { ...clean, missing: 'toToken', error: 'That token pair is not currently supported. Choose USDC, EURC, cirBTC, tUSDC or tARC.' }; }
    case 'sourceChain': { const chain = matchBridgeChain(text); if (!chain) return { ...clean, missing: 'sourceChain', error: 'Please provide a supported chain name.' }; if (!clean.sourceChainId) return { ...clean, sourceChain: chain.name, sourceChainId: chain.chainId, missing: undefined }; if (clean.sourceChainId === chain.chainId) return { ...clean, missing: 'sourceChain', error: 'Destination must be different from the source chain.' }; return { ...clean, destinationChain: chain.name, destinationChainId: chain.chainId, missing: undefined, error: undefined }; }
...
export function clarificationQuestion(pending: PendingFinancialAction): string {
  ...
  switch (pending.missing) {
    ...
    case 'sourceChain': return pending.sourceChain ? `Source is ${pending.sourceChain}. Which destination chain should I use? Supported: ${chains}.` : `Which source and destination chains should I use? Supported: ${chains}.`;
    default: return 'Could you clarify that?';
  }
}

export function describeIntent(intent: PendingFinancialAction): string {
  if (intent.action === 'transfer') return `Send ${intent.amount} USDC to ${intent.recipient?.slice(0, 6)}…${intent.recipient?.slice(-4)}`;
  if (intent.action === 'swap') return `Swap ${intent.amount} ${intent.fromToken} for ${intent.toToken}`;
  if (intent.action === 'bridge') return `Bridge ${intent.amount} USDC from ${intent.sourceChain ?? 'source chain'} to ${intent.destinationChain ?? 'destination chain'}`;

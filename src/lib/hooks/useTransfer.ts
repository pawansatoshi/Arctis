...
import { PRIMARY_CONTRACT, PRIMARY_DECIMALS, ERC20_ABI, CHAIN_ID, RPC_URL, NETWORK_NAME, NETWORK } from '@/lib/contracts';
...
async function preflight(address: `0x${string}`, recipient: `0x${string}`, amount: string) {
  const c = createPublicClient({ chain: arcTestnet, transport: http(RPC_URL) });
  const req = parseUnits(amount, PRIMARY_DECIMALS);
  ...
  if (usdc < req) {
    throw new Error(
      `Insufficient USDC. You have ${formatUnits(usdc, PRIMARY_DECIMALS)} USDC, but ${amount} USDC is required. No transaction was submitted.`,
    );
  }

  if (native < requiredGas) {
    throw new Error(
      `Insufficient USDC for network fees. Approximately ${formatEther(requiredGas)} USDC is required for gas. No USDC was sent.`,
    );
  }
}
...
export function useTransfer(mode: TransactionRecord['mode'] = 'manual') {
  ...
  if (receiptError && txHash && localId.current && !isSuccess) {
    ...
    patch({
      ...
      log: { level: 'error', message: 'Transaction failed on-chain', walletAddress: address },
    }, authHeaders.current);
    toast.dismiss(txHash);
  ...
  const transfer = useCallback(async ({ to, amount, note }: TransferParams) => {
    ...
    try {
      ...
      if (resolved.toLowerCase() === address.toLowerCase()) {
        throw new Error('Recipient must be different from the sending wallet.');
      }
      ...
      await preflight(address, resolved as `0x${string}`, amount);

      const rawAmount = parseUnits(amount, PRIMARY_DECIMALS).toString();
      ...
      toast.success('Transaction submitted — waiting for blockchain confirmation', { id: hash, duration: Infinity });
  ...
  }, [address, connector, chainId, switchChainAsync, addTransaction, updateTransaction, isPending, mode, getAuthHeaders]);

...
import { CHAIN_ID, NETWORK_NAME, EXPLORER_URL } from '@/lib/contracts';
...
export async function logTreasuryEvent(
  type: TreasuryLog['type'], amount: number, description: string,
  walletAddress?: string, txHash?: string,
  meta?: { explorerUrl?: string; networkName?: string; chainId?: number; blockNumber?: string; gasUsed?: string }
): Promise<void> {
  const db = getAdminDb();
  await db.collection(LOGS_COL).doc().set({
    ...
    txHash: txHash ?? null,
    explorerUrl: txHash && (meta?.explorerUrl ?? EXPLORER_URL) ? `${meta?.explorerUrl ?? EXPLORER_URL}/tx/${txHash}` : null,
    networkName: meta?.networkName ?? NETWORK_NAME,
    chainId: meta?.chainId ?? CHAIN_ID,
...
export async function aggregateTreasuryMetrics(): Promise<{ membershipRevenue30d: number; creditRevenue30d: number; aiSpend30d: number; txCount30d: number }> {
  const db = getAdminDb();
  const since = new Date();
  since.setDate(since.getDate() - 30);
  try {
    const snap = await db.collection(LOGS_COL).where('createdAt', '>=', since).orderBy('createdAt', 'desc').limit(500).get();
    let membershipRevenue30d = 0, creditRevenue30d = 0, aiSpend30d = 0, txCount30d = 0;
    snap.docs.forEach((d) => {
      const data = d.data(); txCount30d++;
